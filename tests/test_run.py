...
def test_aras_get(respx_mock, tmp_path):
    ...
    mock_files = [
        mets_file.format(id=0, mime_type="text/plain", size=len(example_content_0.encode("utf8")), href=href_0),
        mets_file.format(id=1, mime_type="text/plain", size=len(example_content_1.encode("utf8")), href=href_1)
    ]
    ...
    runner = CliRunner()
    result = runner.invoke(aras_cli, ['--base', test_base_url, 'get', '--repository', repository, '--path', tmp_path, idn])
